...
export default class InterestsCards extends Component {
    ...
    if (Submit) {
      if (count > 2) {
        ...
        AsyncStorage.getItem("FBAccessUserID").then(value => {
          ...
          id = value;
          axios
            .patch(`http://35.189.6.233:4000/user/${id}`, [
              { propName: "tags", value: pramsCat }
            ])
            .then(res => {
              console.log("Doneee value ", pramsCat);
              console.log("Res ", res);
              AsyncStorage.setItem("UpdateInterests", "true");
              console.log("Interest");
              //   this.props.navigation.navigate('Home');
            });
        });
      } else {
        Alert.alert("Warning", "Select at least 3 categories");
      }
    } else {
  ...
  handleChange = (key, checked) => {
    ...
    if (checked == false) {
      tempSelectedArray.map((item, index) => {
        if (item.category == newArray[key].category) {
            console.log("Remove index ",index)
            tempSelectedArray.splice(index,index);
        }
      });
    } else {
      tempSelectedArray[this.state.selectedList.length] = newArray[key];
    }
    this.setState({ data: newArray, selectedList: tempSelectedArray }, () => {
    ...
    AsyncStorage.getItem("FBAccessUserID").then(value => {
      axios
        ...
        .then(res => {
          ...
          let userTagsArray = userTags.split("|");
          console.log(userTagsArray);

          userTagsArray.map((item, index) => {
            this.props.data.map((itemTep, indexTep) => {
              if (item == itemTep.category) {
                this.handleChange(indexTep, true);
              }
            });
          });
