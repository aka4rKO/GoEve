...
class RBM(object):
    ...
    def __init__(self, visibleDimensions, epochs=20, hiddenDimensions=50, ratingValues=5, learningRate=0.001, batchSize=100):
        # Rating Value was updated from 10 to 5 as there are no half stars
    ...
    def Train(self, X):
        ...
        self.MakeGraph()
        # MakeGraph Function contains the graph itself
    ...
    def GetRecommendations(self, inputUser):
        # inputUser is an array of a particular user rating for movies         
        hidden = tf.nn.sigmoid(tf.matmul(self.X, self.weights) + self.hiddenBias)  # Forward Pass
        visible = tf.nn.sigmoid(tf.matmul(hidden, tf.transpose(self.weights)) + self.visibleBias) # Backward Pass
    ...
    def MakeGraph(self):
        ...
        # Method of randomization selected in relavant to the activation function selected
        maxWeight = -4.0 * np.sqrt(6.0 / (self.hiddenDimensions + self.visibleDimensions))
        # inserts the random weights to the visible and hidden nodes
        ...
        #initializing all hidden bias to zero
        ...
        # Sample from all of the distributions (h sample output from forward pass)
        ...
        # Running the forward and backward passes, and update the weights
